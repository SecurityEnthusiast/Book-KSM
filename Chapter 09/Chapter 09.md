...
# that signed the certificates it revokes, so the root has to sign its own
# list separately: that is `root-crl` on rootca, a machine kept switched
# off. crl-refresh refuses to run without it. What stays unsolved is the
# response time of that ceremony. See OT-035.
