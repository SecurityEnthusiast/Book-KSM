...
| **11** | [Nobody is watching](Chapter%2011/Chapter%2011.md) | Three commands have to happen forever and nothing runs any of them. Worse, nothing would tell you: the first symptom of a broken pipeline is the outage. | A list with a ninety second fuse that passes every check on its way in; three clocks and the two different numbers they produce; a health endpoint that finally answers a question after ten chapters of lying; and a scheduled job that fails every ten minutes and leaves no trace anywhere at all. |
| **12** | [The application stops holding a password](Chapter%2012/Chapter%2012.md) | Eleven chapters of certificates, and the one connection that matters still logs in with a shared value that both ends must know. | A certificate that names a workload rather than a machine; an authority that refuses a legitimate request for the first time; a database that becomes the estate's second verifier and refuses a revoked client during the handshake; and two passwords, two roles and a six-step rotation retired, because the constraint that required them is gone. |
| **13** | [The folder catches up](Chapter%2013/Chapter%2013.md) | Nothing has been rebuilt since Chapter 07, because one machine holds a key that cannot be exported. Six recipes have been quietly falling behind ever since, and the next chapter is a rebuild. | Five recipes made true, including a service missing since Chapter 02; the one machine that never drifted, and the reason; a freshly built image that reproduces every fact about the substrate and cannot sign anything; and a read-only census that refuses to start the offline root in order to describe it. |
| **14** | [Never trust, always verify](Chapter%2014/Chapter%2014.md) | A certificate is checked once, when the connection opens, and then trusted for as long as that connection lives. Revoke it and the application keeps working. | A revocation pushed through the whole pipeline while the application it revokes goes on serving; a database that had been fetching revocation lists for two chapters and honouring none of them; a connection lifetime that turns verification back into an event; an authority that refuses the only machine allowed to talk to it; and four open problems that turn out to have been solved already, two of them closed on the spot and one of those wrongly promised to Kubernetes. |
